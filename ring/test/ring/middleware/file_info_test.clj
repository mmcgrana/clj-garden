(ns ring.middleware.file-info-test
  (:use clj-unit.core ring.middleware.file-info)
  (:import java.io.File))

(def non-file-app (wrap (constantly {:headers {} :body "body"})))

(def known-file (File. "test/ring/middleware/assets/plain.txt"))
(def known-file-app (wrap (constantly {:headers {} :body known-file})))

(def unknown-file (File. "test/ring/middleware/assets/random.xyz"))
